...
1.  **Modify the `.h` file first.** All new events, or changes to existing ones, should be implemented in `src/xDuinoRails_xTrainAPI.h`.
2.  **Update the derivatives.** Propagate the changes to the `.xsd`, `openapi.yaml`, and `src/xDuinoRails_xTrainAPI_utils.h` files to reflect the modifications made to the header.
