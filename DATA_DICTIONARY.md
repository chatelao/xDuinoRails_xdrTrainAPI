Here is the complete **Data Dictionary** of the `UnifiedTrainAPI` as a GitHub-optimized Markdown table. You can copy this block directly into your `README.md` or wiki page.
...
| **System** | `booster` | `bool` | Power supply capability. | `true` = Is booster<br>`false` = Logic only | Filter for UI (Power Management). |
...
| **Telemetry** | `DecoderOrientation`| `enum` | On-track orientation of the locomotive (RCN-217). | `NORMAL` (Chimney first)<br>`INVERTED` (Tender first)<br>`UNKNOWN` | Critical for push-pull train control and automation. |
...
| **Telemetry** | `TelemetryType` | `enum` | Type of measurement. | `SPEED`, `LOAD`, `VOLTAGE`, `CURRENT`, `FUEL`, `TEMP`, `QOS`, `ODOMETER` | Defines the unit of `value`. |
...
| **Config** | `cvNumber` | `int` | Index of the configuration variable. | `1` to `1024` (DCC)<br>`1` to `65535` (Index) | Address in the decoder memory. |
...
| **Config** | `domain` | `string` | Type of the mass data block. | "ICON", "FIRMWARE", "MFX_CONFIG" | Context for `onConfigBlockLoaded`. |
| **Config** | `operation` | `string` | Name of the running process. | e.g., "Reading CV List" | For progress bars/UI. |
| **Config** | `percent` | `float` | Progress. | `0.0` to `100.0` | UI Feedback. |
| **Config** | `success` | `bool` | Result of an operation. | `true` = ACK received<br>`false` = Timeout/NACK | Success control for CV reads. |
